...
import {addBoundary, addDevBoundary} from '../../../../controllers/BoundaryMappingServices';
...
function DevMarkBoundary() {
  ...
  const [isCompletedTrue , setCompletedTrue] = useState(false);
  ...
  useEffect(() => {
    realm.subscriptions
      .update(mutableSubs => {
        mutableSubs.add(realm.objects('BoundaryMappingUpdated'));
      })
      .catch(err => {
  ...
  const handleAddBoundary = useCallback(() => {
    addDevBoundary(
      ...
      user?.fieldSite,
      JSON.stringify(isCompletedTrue)
    );
  }, [
    realm,
    user?.user?.id,
      ...
      {isTripPaused && (
        <View
          ...
          <TouchableOpacity
            ...
            onPress={saveInDrafts}>
            <Text>{'Save in Drafts'}</Text>
          </TouchableOpacity>
        </View>
      )}
...
export default DevMarkBoundary;
